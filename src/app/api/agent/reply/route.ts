...
export async function POST(req: NextRequest) {
  try {
    ...
    const { contact_id, incoming_message, conversation_history = [], user_id: rawUserId = 'philip' } = body;
    ...
    if (supabase && contact_id) {
      ...
      const { data: interactions } = await supabase.from('interactions').select('*').eq('contact_id', contact_id).order('created_at', { ascending: false }).limit(10);
      if (interactions && interactions.length > 0) history = interactions;
    }
